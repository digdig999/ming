<template>
  <div>
    <!-- <cl-header :title="title" :step="step" ref="head"></cl-header> -->
    ...
    <div v-show="bindShow" @click="bindShow=!bindShow" class="cart-back"></div>
    <div class="bind" v-show="bindShow">
      ...
      <div class="btn">
        ...
        <span class="bgColor" @click="goBind()">前往</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    //获取号码
    this.userId = localStorage.getItem("uuid");
    console.log("band",this.$route.query.band)
    if (this.$route.query.band != undefined) {
      this.band=this.$route.query.band;
    } else {
